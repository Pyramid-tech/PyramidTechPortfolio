/**
 * WebGL fluid-simulation cursor trail.
 *
 * Faithful TypeScript port of the original shader implementation: behavior is
 * unchanged. The GL/GLSL plumbing is typed where it is cheap and uses narrow
 * `any`/assertions where fully typing the dynamic WebGL glue would add no value.
 *
 * `initCursor()` now returns a disposer that cancels the animation frame loop
 * and removes the global event listeners, so React can clean it up on unmount.
 */
...
const initCursor = (): Disposer => {
  ...
  function getWebGLContext(canvas: HTMLCanvasElement): { gl: WebGL2RenderingContext; ext: ExtFormats } {
    ...
    const glc = gl as WebGL2RenderingContext;
    ...
    if (isWebGL2) {
      ...
      halfFloat = glc.getExtension('OES_texture_half_float') as { HALF_FLOAT_OES: number } | null;
